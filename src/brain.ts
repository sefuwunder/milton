...
async function unknownReply(session: Session, intent: Intent): Promise<Reply> {
  ...
  // miss-log DB may not be initialized in some test harnesses, and a logging
...
async function playbookEventFollowThrough(event: string, data: Record<string, any>): Promise<void> {
  ...
  // Surface suggestions as toasts via the existing automation-run SSE channel
  // the frontend already renders (kind "reminder" → "⏰ Reminder: …").
  for (const s of res.suggestions) auto.broadcastSse("automation-run", { kind: "reminder", summary: s.text });
}
