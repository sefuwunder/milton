...
export function parseIntent(raw: string): Intent {
  ...
  else if ((m = cased.match(/^(build|make|create|design)( me)?( a| an)? widgets?( (for|that|which|with|to|showing) (?<desc>.+))?$/i))) set("build_widget", { description: (m.groups?.desc || "").trim() });
  else if ((m = cased.match(/^(i want|i need|i'd like|i would like)( a| an)? widgets?( (for|that|which) (?<desc>.+))?$/i))) set("build_widget", { description: (m.groups?.desc || "").trim() });
  else if (/^new widgets?$/.test(text)) set("build_widget", { description: "" });
...
export const HELP_LEVELS: HelpLevel[] = [
  {
    ...
    rows: [
      ...
      { cmds: [["build a widget that shows stalled deals", "build_widget"], ["new widget", "build_widget"]], note: "draft a dashboard widget with me — I file it as a proposal, you preview and approve" },
