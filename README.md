...
**Outcome definitions** — the doc's **Create outcome** / **Activate outcome** screens, as chat. `create outcome` walks the whole screen: name → Contact Cycle Action (Recycle / Complete / No effect / Schedule appointment) → the conditional field (Default Recycle Period in days, or Completion Reason) → Functions (Reassign / Redirect / Pipeline, multi-select) → Applicable actions (Email / In person / Review / Snail mail / Text message / Voice call, multi-select) → a CONFIRM card before anything is saved. Definitions are workspace-scoped (`outcomes`, `show outcome <name>`, `delete outcome <name>` asks first). `activate outcome <name> for <deal>` walks the Activate screen: recycle date (or the default), completion reason, appointment start/end with validation, then the chosen functions (new owner, redirect contact, pipeline stage) and an optional note, then SUBMIT. Submitting logs the outcome to Milton's outcome log (so Review → Action → Outcome rules see it) and applies the effects to exec-crm: recycle creates a follow-up task, complete moves the deal to won/lost, schedule-appointment creates an appointment task, reassign/redirect/pipeline patch the deal. `No` cancels at any step and nothing is written.
